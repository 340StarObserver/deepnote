    ...
    def perform(self):
        ...
        if exist_note(db_conn,self._server_conf['mongo']['db_name'],note_id) is None:
            ...
            if ever_action is None:
                ...
                # increase this note's agree_num
                affect_note(db_conn,self._server_conf['mongo']['db_name'],note_id,'agree_num',1)
                # add record that this user agree this note
                agree_oppose_add(db_conn,self._server_conf['mongo']['db_name'],\
                    self._usr_session['user_id'],note_id,0)
            ...
            elif ever_action is 0:
                # if ever has agreed this note
                response['result'] = True
                # decrease this note's agree_num
                affect_note(db_conn,self._server_conf['mongo']['db_name'],note_id,'agree_num',-1)
                # delete the ever agree record
                agree_oppose_delete(db_conn,self._server_conf['mongo']['db_name'],\
                    self._usr_session['user_id'],note_id,0)
            else:
                # ever has opposed this note
                response['reason'] = 3
